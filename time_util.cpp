...
const int64_t TimeUtil::MIN = INT64_MIN;
const int64_t TimeUtil::MAX = UINT64_MAX;
const int64_t TimeUtil::PRE_MAX = UINT64_MAX - 1;
...
inline time_t TimeUtil::getTime()
{
    struct timeval tval;
    gettimeofday(&tval, NULL);
...
timeval TimeUtil::getTimeval()
{
    struct timeval tval;
    gettimeofday(&tval, NULL);
    return tval;
}
timeval TimeUtil::getTimeval(int64_t usecOffset)
{
    timeval tval;
...
tm *TimeUtil::gmtime_r(time_t *gmt, tm *tmspace)
{
...
int TimeUtil::parseGMTDate(const char *date, int roundup,tm *result)
{
...
time_t TimeUtil::parseGMTDate(const char *value)
{
    tm mytm;
...
char *TimeUtil::toGMTTimeString(char buf[30], time_t now)
{
    tm *ltm;
    tm tmspace;
...
char *TimeUtil::logLocalTimeString(char *buf)
{
    char sign;
    static const char *Mon[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                   "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
    time_t now =getTime();
    struct tm  gtm;
    TimeUtil::gmtime_r(&now, &gtm);
    struct tm  ltm;
    TimeUtil::localtime_r(&now, &ltm);
